    ...
    <form onSubmit={submit} className="rounded border p-4 space-y-3 bg-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input className="w-full border rounded p-2" value={description}
            onChange={(e) => setDescription(e.target.value)} placeholder="e.g., Lunch with client" />
        </div>
      </div>

      <button type="submit" className="bg-black text-white rounded px-4 py-2 hover:opacity-90">Add</button>
    </form>
  );
}